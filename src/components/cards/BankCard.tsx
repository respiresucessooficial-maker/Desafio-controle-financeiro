  ...
  return (
    <div className="flex flex-col" onClick={onClick} style={{ cursor: onClick ? 'pointer' : undefined }}>
      <motion.div
        ...
        className={`relative select-none rounded-3xl bg-linear-to-br ${bank.color} ${
          compact ? 'h-30 w-50' : 'h-41.25 w-70'
        }`}
      >
        <div className={`absolute inset-0 overflow-hidden rounded-3xl ${compact ? 'p-4' : 'p-6'}`}>
          <div
          ...
          <div className="relative z-10 flex h-full flex-col justify-between">
            <div className="flex items-start justify-between">
              <div>
                ...
                {!compact && (
                  <p className={`mt-0.5 text-xs opacity-60 ${bank.textColor}`}>
                    Agencia {bank.code}
                  </p>
                )}
              </div>
            ...
            <div>
              {!compact && (
                <p className={`mb-0.5 text-[10px] uppercase tracking-wider opacity-60 ${bank.textColor}`}>
                  Limite disponivel restante
                </p>
              )}
              <p className={`font-bold ${compact ? 'text-base' : 'text-xl'} ${bank.textColor}`}>
                {formatter.format(availableRemaining)}
              </p>
            </div>
          </div>
        </div>
      ...
      {showCreditBar && (availableConfigured > 0 || approvedLimit > 0) && (
